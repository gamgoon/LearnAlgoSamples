...
void Huffman_Encode(HuffmanNode** Tree, UCHAR* Source, BitBuffer* Encoded, HuffmanCode CodeTable[MAX_CHAR])
{
	...
	while (Source[i] != '\0')
	{
		...
		{
			Huffman_AddBit(Encoded, CodeTable[Source[i]].Code[j]); // 인코드된 코드값 생
		}
...
void Huffman_BuildPrefixTree(HuffmanNode** Tree, SymbolInfo SymbolInfoTable[MAX_CHAR])
{
	...
	for (i=0; i<MAX_CHAR; i++)
	{
		...
		{
			HuffmanNode* BitNode = Huffman_CreateNode(SymbolInfoTable[i]); // 트리의 노드 생성 (최초의 잎노드 생성)
			PQNode NewNode;
			NewNode.Priority = SymbolInfoTable[i].Frequency;
			NewNode.Data = BitNode;
			PQ_Enqueue(PQ, NewNode); // 해당 트리노드와 빈도를 뷰에 삽입
		}
	}
	...
	while (PQ->UsedSize > 1)
	{
		...
		HuffmanNode* BitNode = Huffman_CreateNode(NewData); // 부모노드 생성
		HuffmanNode* Left;
		HuffmanNode* Right;
		...
		BitNode->Data.Frequency = Left->Data.Frequency + Right->Data.Frequency; // 부모노드의 빈도 (자식노드의 빈도의 합)
		...
		PQ_Enqueue(PQ, NewNode); // 생성된 부모노드는 다시 큐에 삽입.
	}

	PQ_Dequeue(PQ, &Result); // 최종 마지막에 남은 노드가 트리의 루트노드임.
...
void Huffman_BuildCodeTable(HuffmanNode* Tree, HuffmanCode CodeTable[MAX_CHAR], UCHAR Code[MAX_BIT], int Size)
{
	...
	if (Tree->Left != NULL)
	{
		Code[Size] = 0; // 경로상의 코드값 오른쪽 자식인 경우 0
		Huffman_BuildCodeTable(Tree->Left, CodeTable, Code, Size + 1);
	}

	if (Tree->Right != NULL)
	{
		Code[Size] = 1; // 경로상의 코드값 오른쪽 자식인 경우 1
	...
	if (Tree->Left == NULL && Tree->Right == NULL)
	{
		...
		for (i=0;i<Size;i++)
		{
			CodeTable[Tree->Data.Symbol].Code[i] = Code[i]; // 루트에서 경로에 있는 노드별 코드로 최종 코드생성.
		}
